...
class DownloadConfig(BaseModel):
    ...
    include_all_tags: bool = Field(
        default=False, description="Download all OSM tags without filtering"
    )
...
class ProcessingConfig(BaseModel):
    ...
    distance: float | None = Field(default=None, description="Buffer distance")
    distance_unit: str = Field(default="meters", description="Distance unit")

    # Spatial join parameters
    other_file: Path | None = Field(
        default=None, description="Other dataset for spatial operations"
    )
    ...
    target_crs: str | None = Field(default=None, description="Target coordinate reference system")
...
class VisualizationConfig(BaseModel):
    ...
    label_field: str | None = Field(default=None, description="Field to use for labeling")
    color_field: str | None = Field(default=None, description="Field to use for color coding")
    size_field: str | None = Field(default=None, description="Field to use for sizing features")
    config_file: Path | None = Field(default=None, description="Custom KeplerGL configuration file")
    ...
    zoom_level: int | None = Field(default=None, ge=0, le=20, description="Manual zoom level")
...
class GeoCLIConfig(BaseModel):
    ...
    max_memory_gb: float | None = Field(
        default=None, ge=0.5, description="Maximum memory usage in GB"
    )
    sedona_memory: str | None = Field(default=None, description="SedonaDB memory setting")
