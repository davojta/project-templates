...
describe("API Layer", () => {
  ...
  it("should insert and retrieve a layer", () => {
    const insertQuery = db.query(
      "INSERT INTO layers (id, name, url, visible) VALUES (?, ?, ?, ?) RETURNING *",
    );
    const layer = insertQuery.get(
      "test-id",
      "Test Layer",
  ...
  it("should insert and retrieve a feature review", () => {
    db.query(
      "INSERT INTO layers (id, name, url, visible) VALUES (?, ?, ?, ?)",
    ).run("layer-1", "Test Layer", "http://example.com/data.geojson", 1);

    const insertReview = db.query(
      "INSERT INTO feature_reviews (feature_id, layer_id, status, reviewed_at) VALUES (?, ?, ?, ?) RETURNING *",
    );
    const review = insertReview.get(
      "feature-1",
      "layer-1",
      "flagged",
