...
function runClaude(
  ...
  try {
    ...
    return {
      output: (e.stdout || e.message || "").trim(),
...
async function eval1_appInspectAvailable(): Promise<EvalResult> {
  ...
  const mapType = surfJs(
    "return typeof window.__appInspect.map?.getFeatureDetails",
  );
  assertions.push({
...
async function eval2_mapFeatureNavigation(): Promise<EvalResult> {
  ...
  const details1Raw = surfJs(
    "return JSON.stringify(window.__appInspect.map.getFeatureDetails())",
  );
...
async function eval3_mapFlagUnflag(): Promise<EvalResult> {
  ...
  if (wasFlagged) {
    ...
    assertions.push({
      check: "unflag() returns true when flagged",
      passed: unflagResult.includes("true"),
      detail: unflagResult,
    });
    sleep(1000);
    ...
    assertions.push({
      check: "isFlagged is false after unflag",
      passed: after?.isFlagged === false,
      detail: `isFlagged=${after?.isFlagged}`,
    });
    // restore
    ...
    assertions.push({
      check: "flag() returns true when unflagged",
      passed: flagResult.includes("true"),
      detail: flagResult,
    });
    sleep(1000);
    const after = parseJson(
    ...
    assertions.push({
      check: "isFlagged is true after flag",
      passed: !!after?.isFlagged,
      detail: `isFlagged=${after?.isFlagged}`,
    });
    // restore
...
async function eval4_crossPageNavigation(): Promise<EvalResult> {
  ...
  assertions.push({
    check: "navigated to table",
    passed: tablePage.includes("table"),
    detail: tablePage,
  });

  const tableDataRaw = surfJs(
    "return window.__appInspect.table.getData().length",
  );
  ...
  assertions.push({
    check: "navigated to results",
    passed: resultsPage.includes("results"),
    detail: resultsPage,
  });

  const summaryRaw = surfJs(
    "return JSON.stringify(window.__appInspect.results.getSummary())",
  );
  ...
  assertions.push({
    check: "navigated back to map",
    passed: mapPage.includes("map"),
    detail: mapPage,
  });
...
async function eval5_mapInspectViewport(): Promise<EvalResult> {
  ...
  const cameraRaw = surfJs(
    "return JSON.stringify(window.__mapInspect?.getCamera())",
  );
  ...
  const layersRaw = surfJs(
    'return JSON.stringify(window.__mapInspect?.getLayers().filter(l => l.id.startsWith("geojson")))',
  );
...
`;
  ...
  const resultJson = resultLine
    ? parseJson(resultLine.replace("RESULT:", "").trim())
    : null;
  ...
  assertions.push({
    ...
    passed:
      typeof resultJson?.thirdFeature?.name === "string" &&
      resultJson.thirdFeature.name.length > 0,
...
`;
  ...
  assertions.push({
    ...
    passed:
      typeof resultJson?.largestCity?.name === "string" &&
      resultJson?.largestCity?.population > 0,
...
async function main() {
  ...
  const runDeterministic =
    args.length === 0 || args.includes("--deterministic");
  ...
  console.log(
    `Results: ${passed} passed, ${failed} failed out of ${results.length} evals`,
  );
